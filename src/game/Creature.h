...
struct VendorItemCount
{
    explicit VendorItemCount(uint32 _item, uint32 _count)
        : itemId(_item), count(_count), lastIncrementTime(sGameTime.GetGameTime()) {}
...
class MANGOS_DLL_SPEC Creature : public Unit
{
    ...
    public:
        ...
        void SetRespawnTime(uint32 respawn) { m_respawnTime = respawn ? sGameTime.GetGameTime() + respawn : 0; }
        void Respawn();
