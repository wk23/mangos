...
void WorldSession::HandleLogoutRequestOpcode( WorldPacket & /*recv_data*/ )
{
    ...
    data << uint8(0);
    SendPacket( &data );
    LogoutRequest(sWorld.GetGameTime());
}
...
void WorldSession::HandleTogglePvP( WorldPacket & recv_data )
{
    ...
    else
    {
        if(!GetPlayer()->pvpInfo.inHostileArea && GetPlayer()->IsPvP())
            GetPlayer()->pvpInfo.endTimer = sWorld.GetGameTime();     // start toggle-off
    }
}
...
void WorldSession::HandleCorpseReclaimOpcode(WorldPacket &recv_data)
{
    ...
    if(corpse->GetGhostTime() + GetPlayer()->GetCorpseReclaimDelay(corpse->GetType()==CORPSE_RESURRECTABLE_PVP) > sWorld.GetGameTime())
        return;
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket & recv_data)
{
    ...
    if(objmgr.IsTavernAreaTrigger(Trigger_ID))
    {
        ...
        GetPlayer()->InnEnter(sWorld.GetGameTime(), atEntry->mapid, atEntry->x, atEntry->y, atEntry->z);
