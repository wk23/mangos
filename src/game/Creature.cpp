...
void Creature::RemoveCorpse()
{
    ...
    loot.clear();
    m_respawnTime = GetGameTime() + m_respawnDelay;
...
bool Creature::LoadFromDB(uint32 guid, Map *map)
{
    ...
    if(m_respawnTime > GetGameTime())                          // not ready to respawn
    {
        m_deathState = DEAD;
        if(canFly())
        {
...
void Creature::Respawn()
{
    ...
    if(getDeathState()==DEAD)
    {
        if (m_DBTableGuid)
            objmgr.SaveCreatureRespawnTime(m_DBTableGuid,GetInstanceId(),0);
        m_respawnTime = GetGameTime();                         // respawn at next tick
    }
}
...
void Creature::SaveRespawnTime()
{
    ...
    if(m_respawnTime > GetGameTime())                          // dead (no corpse)
        objmgr.SaveCreatureRespawnTime(m_DBTableGuid,GetInstanceId(),m_respawnTime);
    else if(m_deathTimer > 0)                               // dead (corpse)
        objmgr.SaveCreatureRespawnTime(m_DBTableGuid,GetInstanceId(),GetGameTime()+m_respawnDelay+m_deathTimer/IN_MILISECONDS);
}
...
void Creature::AddCreatureSpellCooldown(uint32 spellid)
{
    ...
    if(cooldown)
        _AddCreatureSpellCooldown(spellid, GetGameTime() + cooldown/IN_MILISECONDS);

    if(spellInfo->Category)
        _AddCreatureCategoryCooldown(spellInfo->Category, GetGameTime());
...
bool Creature::HasCategoryCooldown(uint32 spell_id) const
{
    ...
    return(itr != m_CreatureCategoryCooldowns.end() && time_t(itr->second + (spellInfo->CategoryRecoveryTime / IN_MILISECONDS)) > GetGameTime());
}

bool Creature::HasSpellCooldown(uint32 spell_id) const
{
    CreatureSpellCooldowns::const_iterator itr = m_CreatureSpellCooldowns.find(spell_id);
    return (itr != m_CreatureSpellCooldowns.end() && itr->second > GetGameTime()) || HasCategoryCooldown(spell_id);
}
