...
{
    const MapEventIdx eventId = MapManager.GetCreatureEventIndex(creature->GetDBTableGUIDLow());
...
uint64 Map::GetSingleCreatureGuid(uint8 event1, uint8 event2)
{
    MapCreatures::const_iterator itr = m_EventObjects[MAKE_PAIR32(event1, event2)].creatures.begin();
...
{
    const MapEventIdx eventId = MapManager.GetGameObjectEventIndex(obj->GetDBTableGUIDLow());
    if (eventId.event1 == MAP_EVENT_NONE)
        return;
    ...
    if (!IsActiveEvent(eventId.event1, eventId.event2))
    {
        ...
        if (IsDoor(eventId.event1, eventId.event2))
...
bool Map::IsDoorEvent(uint8 event1, uint8 event2)
{
...
void Map::OpenDoorEvent(uint8 event1, uint8 event2 /*=0*/)
{
    ...
    MapObjects::const_iterator itr = m_EventObjects[MAKE_PAIR32(event1, event2)].gameobjects.begin();
...
void Map::SpawnEvent(uint8 event1, uint8 event2, bool spawn)
{
    ...
    MapObjects::const_iterator itr2 = m_EventObjects[MAKE_PAIR32(event1, event2)].gameobjects.begin();
