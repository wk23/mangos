...
bool LoginQueryHolder::Initialize()
{
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADMAILCOUNT,       "SELECT COUNT(id) FROM mail WHERE receiver = '%u' AND (checked & 1)=0 AND deliver_time <= '" I64FMTD "'", GUID_LOPART(m_guid),(uint64)GetGameTime());
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder * holder)
{
    ...
    data << uint32(GetGameTime());                             // unix time of something
    data << uint8(1);
