...
bool ChatHandler::HandleNpcInfoCommand(const char* /*args*/)
{
    ...
    int32 curRespawnDelay = target->GetRespawnTimeEx()-sWorld.GetGameTime();
...
bool ChatHandler::HandleBanInfoHelper(uint32 accountid, char const* accountname)
{
    ...
    do
    {
        ...
        bool active = false;
        if(fields[2].GetBool() && (fields[1].GetUInt64() == (uint64)0 ||unbandate >= sWorld.GetGameTime()) )
            active = true;
...
bool ChatHandler::HandleInstanceListBindsCommand(const char* /*args*/)
{
    ...
    for(uint8 i = 0; i < TOTAL_DIFFICULTIES; i++)
    {
        ...
        for(Player::BoundInstancesMap::iterator itr = binds.begin(); itr != binds.end(); ++itr)
        {
            ...
            std::string timeleft = GetTimeString(save->GetResetTime() - sWorld.GetGameTime());
